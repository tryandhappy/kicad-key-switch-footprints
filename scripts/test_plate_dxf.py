...
実行: python3 scripts/test_plate_dxf.py
"""
...
def build_plate():
    ...
    return plate.simplify(0.002, preserve_topology=True), cuts
...
def write_svg(plate, path):
    ...
    labels = [
        ...
        (-3, 1, "2u: Gateron LP stab | Choc V2 stab | Choc V1 stab | 14.00"),
        (-3, 2, "2u vertical: Gateron LP | Choc V2 | Choc V1 ; 14.00 x8"),
    ]
    txt = "".join(
    ...
    svg = (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="-80 -80 160 160" width="960" height="960">'
           ...
           f'test plate 150x150 mm, 19.05 pitch 7x7, mounting holes d2.2 at (+-70,+-70). '
...
def main():
    ...
    write_dxf(plate, OUT_DXF)
    write_svg(plate, OUT_SVG)
    n_holes = len(plate.interiors)
    print(f"DXF: {OUT_DXF.relative_to(ROOT)}  SVG: {OUT_SVG.relative_to(ROOT)}")
    print(f"外形 {PLATE}x{PLATE} mm、切り抜き {n_holes} 個、"
