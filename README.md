...
- **四隅リリーフ（dogbone）**: スイッチ保持用の `User.2/3/4` は、正方形の各角を中心に
  ...
  - `_MXPCBStab` の 2u 系スタビ開口（内側の辺 x=±8.563）とは重ならないが、間の梁は
    ...
    （y=7.58〜8.98）には下側 2 隅のリリーフ円（最下点 y=7.975）が 0.39mm 食い込み、
    開口下辺と溝の間の 0.6mm の帯が孤立した島になる**（2026-09-07 判明。
    詳細と図は下の `_ChocV2Stab` の警告を参照）。
...
- **スタビライザー**（2u 以上と ISO Enter）:
  ...
  - **Kailh Choc V2 スタビ（CPG1353G24D01）用 → `_ChocV2Stab` 版**（2.00u のみ =
    ...
    **⚠ FR4 プレートでの強度警告（2026-09-07）**: この形状は 1.2mm FR4 プレートには向かない。
    ...
    - **開口下辺と溝の間の 0.6mm の帯は島になる**。現状の User.3 は四隅リリーフ R1.0
      （角中心の円）で、下側 2 隅の円が溝の上端より 0.39mm 下まで届くため、帯は上を開口・
      下を溝・両端を円に囲まれて孤立し、加工すると脱落する（結果として開口下辺が y=8.98 まで
      開く）。元の kb-plategen はリリーフ無しの正方形で帯が両端でつながる設計だが、
      0.6mm 幅の FR4 は公差 ±0.2mm で 0.4mm まで痩せるので、つながっていても実用強度は無い
    - **対策候補（未決定、図の②③）**: 案 1 = `_ChocV2Stab` 版だけ下側 2 隅を T ボーン
      （円の中心を辺に沿って 1.0 上にずらし (±6.975, 5.975) に置く。円の最下点が開口下辺と
      同じ高さになり溝に届かない。帯は残るが島にはならない）、
      案 2 = `_ChocV2Stab` 版の User.3 下辺を溝の下端 y=8.981 まで下げて一体の開口にする
      （帯も島も無く加工は確実。スイッチ下辺の保持面が無くなる）。**どちらも片持ち梁は残る**
